...
var Sea = function(){
  ...
  // UV 솔기처럼 XYZ는 같지만 UV가 달라 병합되지 않은 버텍스도
  // 동일한 wave 속성을 공유하도록 위치 기반 맵으로 관리
  var posMap = {};
  for (var i = 0; i < l; i++) {
...
Sea.prototype.moveWaves = function (){
  var posAttr = this.mesh.geometry.attributes.position;
  var l = posAttr.count;

  // 1패스: 위치 계산 (ang은 아직 수정하지 않음)
  for (var i=0; i<l; i++){
    var vprops = this.waves[i];
    posAttr.setX(i, vprops.x + Math.cos(vprops.ang)*vprops.amp);
    posAttr.setY(i, vprops.y + Math.sin(vprops.ang)*vprops.amp);
  }

  // 2패스: 고유 vprops 객체마다 한 번만 ang 증가
  var updated = new Set();
  for (var i=0; i<l; i++){
    var vprops = this.waves[i];
    if (!updated.has(vprops)){
