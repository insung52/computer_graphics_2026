...
var Sea = function(){
  ...
  // per-vertex 어트리뷰트 배열 (UV 솔기 중복 버텍스는 동일 값 공유)
  ...
  // onBeforeCompile: Phong 조명은 그대로, position만 shader에서 변위
    ...
    shader.vertexShader =
      ...
      shader.vertexShader.replace(
        ...
        [
          ...
          'transformed.x += cos(wAngle) * waveAmp;',
          'transformed.y += sin(wAngle) * waveAmp;',
        ].join('\n')
      );
...
Sea.prototype.updateTime = function(dt){
  if (this.mesh.material.userData.shader) {
    this.mesh.material.userData.shader.uniforms.time.value += dt;
  }
}
